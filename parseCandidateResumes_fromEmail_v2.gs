...
var subjectSearchString = 'Candidate Submission OR candidate submittal';
...
function getEmailThreadsBySubjectAndSender(searchString,arr){ 
  var matches = [];
  ...
  for(var i=0; i<threads.length; i++){
    var msgs = threads[i].getMessages(); //https://developers.google.com/apps-script/reference/gmail/gmail-thread#getmessages
    var subject = msgs[0].getSubject(); //https://developers.google.com/apps-script/reference/gmail/gmail-message#getsubject
    var sender = msgs[0].getFrom(); //https://developers.google.com/apps-script/reference/gmail/gmail-message#getFrom()
...
*/
    if(msgs[0].isUnread() && booleanSearch(searchString,subject) && arr.some(function(el){return new RegExp(el,'i').test(sender)})) { 
     matches.push(msgs[0]); 
...
function parseCandidateSubmissions(){
  var senderEmails = getTargetEmails();
  var targetEmails = getEmailThreadsBySubjectAndSender(subjectSearchString,senderEmails);
  for(var i=0; i<targetEmails.length; i++){
    ...
    for(var a=0; a<attachments.length; a++){
        ...
        var newDocFileBodyText = newDocFile.getBody().getText().replace(/^\d+\/\d+\/\d+\s*/, '');
